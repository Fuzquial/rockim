...
NIT = int(sys.argv[2]) if len(sys.argv) > 2 else 200
ETA = float(sys.argv[3]) if len(sys.argv) > 3 else 0.6
...
def run(a_excl, kappa, lloyd, seedno=1):
    ...
    w = (kappa * sp) ** 2
    aT = W * H * sp ** 2 / (sp ** 2).sum()          # aires PRESCRITES, somme = W H
    err = None
    for it in range(lloyd + 1):
        hist = []
        for k in range(NIT):                        # iteration duale (Bourne 2020)
            cells = laguerre(seeds, w)
            A = np.array([area_centroid(c)[0] for c in cells])
            w = w + ETA * (aT - A) / (math.pi / 2)
            err = (np.abs(A - aT) / aT).max()
            if k in (9, 39, 99, NIT - 1): hist.append(f"{k+1}:{100*err:.1f}%")
        cells = laguerre(seeds, w)
        print("      convergence", " ".join(hist), " vides:", sum(len(c) < 3 for c in cells))
        if it == lloyd: break
    ...
    return dict(placed=len(seeds), target=target, skipped=skipped, sd=lnd.std(), dmean=deq.mean() * 1e3,
                corr=corr, cover=areas.sum() / (W * H), err=err)


print(f"sigma demande = {sigma} ; s = {s*1e3:.3f} mm ; cible d_eq moyen ~ {d*1e3:.1f} mm x exp(sigma^2/2)")
print(f"{'a_excl':>7s} {'kappa':>6s} {'lloyd':>5s} {'place/vise':>11s} {'sd ln d':>8s} {'corr':>6s} {'d_eq mm':>8s} {'err max':>8s}")
for a_excl, kappa in [(0.35, 0.315)]:
    for lloyd in (0, 2):
        r = run(a_excl, kappa, lloyd)
        print(f"{a_excl:7.2f} {kappa:6.3f} {lloyd:5d} {r['placed']:5d}/{r['target']:<5d} {r['sd']:8.3f} {r['corr']:6.2f} {r['dmean']:8.2f} {100*r['err']:7.2f}%")
