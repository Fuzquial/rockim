...
| **`contact`** (penalty) | penalty \| **potential** = contact général par **potentiel de Munjiza** (éq. 2-5 de Yan et al. 2023), **2D seulement** (portage 3D = phase 2). Paires d'**éléments** (et non nœud-arête) : force normale distribuée F = p·∮(φ_A−φ_B)·n dΓ sur le bord du recouvrement des deux triangles, φ = 3·min(barycentriques), intégration **exacte** (subdivision aux médianes), 3e loi de Newton **machine**, champ **conservatif** — collision élastique : ΔKE/KE₀ = 3,7e-12, transfert exact (selftest-potential2d). Frottement tangentiel incrémental à ressort + cap de Coulomb (éq. 4-5), historique par paire. Détection O(N) type NBS (binning AABB), exclusion des paires liées par un joint **vivant**, compose avec `gcActivation`/`gcXwindow`. Relève de naissance par **aire** de recouvrement (pen0_ du potentiel, τ = `gcBirthTau`) : une paire née en recouvrement (joint mort comprimé) ne matérialise pas son énergie potentielle — signe absorbant garanti (une rampe temporelle ferait l'inverse, mesuré). ⚠️ le gcWork peut porter un petit résidu positif (biais O(dt) du compteur + relève) — annoté dans le résumé, pas une pathologie en potentiel. L'outil analytique reste en pénalité (levé par B1). SHPB : onde incidente identique au penalty à 3e-6 près ; zone broyée **conservative** → rebond plus élastique (percussion 2D : e 0,55 → 0,71, moins de casses) — écart de loi physique assumé |
| `potPenaltyFactor` (1.0) | pénalité normale du potentiel, en multiples de E·épaisseur |
| `potTangentFactor` (1.0) | raideur tangentielle de l'éq. 4-5, en multiples de E·épaisseur |
