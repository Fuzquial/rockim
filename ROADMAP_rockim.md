...
- [x] **`tools/verify_suite.py`** — runner de non-régression (fast/full/all, charge nulle, dampWork ≤ 0), 17/17 PASS
...
| **A2** | **Décharge en cisaillement sur sécante à l'origine** (éq. 18) — aujourd'hui plasticité à retour, identique en charge monotone | très court | ~40 lignes + états s_max/τ_max |
...
1. **A1 — activation adaptative du contact** : le plus court des chantiers à fort rendement, dans la continuité directe de ce qui vient d'être fait.
2. **B4 — bilan d'énergie par sous-système** : zéro risque, sert deux objectifs à la fois.
