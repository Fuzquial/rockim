...
#   python3 tools/verify_suite.py ... --update-refs refs_macos_arm.json  # écrire la baseline de CETTE plateforme
#   python3 tools/verify_suite.py ... --refs refs_macos_arm.json         # certifier contre elle
#   (seules les références CHIFFRÉES sont rebasables ; les invariants —
#    charges nulles, dampWork <= 0, compteurs, selftests — restent en dur)
#
...
# ---------------------------------------------------------------------------
import argparse, json, os, platform, re, subprocess, sys, tempfile, time
...
def run_one(exe, t, outroot, env, timeout, refs=None):
    ...
    for (kind, ref, tol, required) in t["checks"]:
        ...
        if kind == "dampwork":                          # <= 0 exigé
            ...
            continue
        # Seuls les contrôles COMPARÉS À UNE RÉFÉRENCE arrivent ici (dampwork,
        # absent:, pass_tag ont déjà `continue`) : ce sont eux, et eux seuls,
        # que --update-refs capture et que --refs peut remplacer.
        meas[kind] = val
        if kind in ovr:
            ref = ovr[kind]
        if ref is None:
    ...
    return dict(name=t["name"], ok=ok, dt=dt, detail=detail, meas=meas)
...
def main():
    ...
    ap.add_argument("--refs", default=None,
                    help="JSON de baseline de plateforme : remplace les références "
                         "CHIFFRÉES ; les invariants (charges nulles, dampWork, "
                         "compteurs, selftests) restent en dur")
    ap.add_argument("--update-refs", default=None, dest="update_refs", metavar="FICHIER",
                    help="écrit les valeurs mesurées de ce run au format --refs "
                         "(baseline de plateforme — à générer sur un run que l'on "
                         "a inspecté, pas pour faire taire un échec)")
    args = ap.parse_args()
    ...
    if args.refs:
        with open(args.refs) as f:
            refs = json.load(f)
        refs.pop("_meta", None)
        print(f"[suite] références de plateforme : {args.refs} ({len(refs)} tests)")
    ...
    results, allok = [], True
    for t in sel:
        r = run_one(args.exe, t, outroot, env, args.timeout, refs)
        results.append(r)
        allok &= r["ok"]
    ...
    if args.update_refs:
        base = {"_meta": dict(platform=platform.platform(),
                              python=platform.python_version(),
                              exe=args.exe, tier=args.tier, threads=args.threads,
                              date=time.strftime("%Y-%m-%d %H:%M:%S"),
                              note="références CHIFFRÉES mesurées sur cette "
                                   "plateforme ; les invariants ne sont pas ici "
                                   "— ils restent en dur dans verify_suite.py")}
        for r in results:
            if r.get("meas"):
                base[r["name"]] = r["meas"]
        with open(args.update_refs, "w") as f:
            json.dump(base, f, indent=1, sort_keys=True, ensure_ascii=False)
        print(f"[suite] baseline écrite : {args.update_refs} "
              f"({len(base) - 1} tests) — certifier ensuite via --refs")
